...
describe('localized application', () => {
  it('keeps locale resources in parity and updates document metadata with a persisted change', () => {
    ...
    expect(document.title).toBe('Solitude — Duelo de Prioridades em Vinil')
  ...
  it('switches immediately without losing the current screen or unsaved form input', async () => {
    ...
    expect(screen.getByRole('heading', { name: /vamos preparar o palco/i })).toBeInTheDocument()
  ...
  it('covers the Portuguese import, review, mode, tie battle, results, track validation, and dialog flow', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /começar um ranking/i }))
    ...
    await user.click(screen.getByRole('button', { name: /adicionar seus discos/i }))
    ...
    expect(await screen.findByRole('heading', { name: /revise seus discos/i })).toBeInTheDocument()
    ...
    await user.click(screen.getByRole('button', { name: /começar duelo equilibrado/i }))
    ...
    expect(await screen.findByRole('heading', { name: /seu próximo disco está decidido/i })).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: /revisar as músicas/i }))
    expect(await screen.findByRole('heading', { name: /guarde as músicas que ficam com você/i })).toBeInTheDocument()
    await user.type(screen.getByLabelText(/total de faixas/i), '1')
    await user.type(screen.getByLabelText(/faixas curtidas/i), '2')
    expect(screen.getByRole('alert')).toHaveTextContent(/não pode superar o total/i)
    await user.clear(screen.getByLabelText(/faixas curtidas/i))
    await user.type(screen.getByLabelText(/faixas curtidas/i), '1')
    await user.click(screen.getByRole('button', { name: /salvar e ir ao próximo álbum/i }))
    ...
    expect(screen.getAllByText(/0,00 coração/i)).toHaveLength(2)
